...
RSpec.describe User, type: :model do
  ...
  describe 'instance methods' do
    ...
    describe 'merchant stats methods' do
      ...
      it ".no_thumbnail - order_items that haven't been fufilled by a merchant" do
        merchant = create(:merchant)
        ...
        item4 = create(:item, image: "https://picsum.photos/200/300?image=4", user: merchant)
        item5 = create(:item, image: 'https://picsum.photos/200/300/?image=524', user: merchant1)

        expect(merchant.no_thumbnail).to include(item1, item2, item3)
        expect(merchant.no_thumbnail.count).to eq(3)
        expect(merchant.no_thumbnail).to_not include(item4, item5)
      end 
      it '.unfulfilled_orders_count - finds count of all unfulfilled orders' do 
        merchant = create(:merchant)
        order = create(:order)
      ...
      it '.unfulfilled_orders_revenue - finds count of all unfulfilled orders' do 
        merchant = create(:merchant)
